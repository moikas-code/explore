...
const AKKORO_LIB = {
  ...
  get_all_collections: async ({sdk}: {sdk: IRaribleSdk}) => {
    return await sdk.apis.collection.getAllCollections({});
  },

  get_ownership_status: async (address: UnionAddress, nft_id: ItemId) => {
  ...
  get_ownership_by_nft_id: async (address: ItemId) => {
    // console.log(address);
  ...
  get_bids_by_nft_id: async (address: ItemId) => {
    // console.log(address);
    const url =
  ...
  get_orders_by_nft_id: async (address: ItemId) => {
  ...
  get_nfts_from_contract_address: async (
    address: UnionAddress = toUnionAddress(
  ...
  get_items_by_owner: async (address: UnionAddress) => {
    try {
      const base = process.env.DEV !== 'true' ? baseURL : dev_baseURL;
  ...
  get_item_by_nft_id: async (nft_id: ItemId) => {
    try {
  ...
  mint: async ({
    ...
    return await AKKORO_LIB.get_collectionByAddress({
      sdk,
      address: toUnionAddress(collection),
    })
      .then(
        async (_collection) =>
    ...
    const orderId = await submit({
      ...
      originFees:
        blockchain == 'ETHEREUM'
          ? [
              {
                account: 'ETHEREUM:0x3E874472Da434f8E1252E95430a65e8F516ED00d' as UnionAddress,
                value: 100,
              },
            ]
          : blockchain == 'TEZOS'
          ? [
              {
                account: 'TEZOS:tz1Q5duBxjCNy1c5Kba63Mf5Jqz9wyKqXFAk' as UnionAddress,
                value: 100,
              },
            ]
          : blockchain == ' FLOW'
          ? [
              {
                account: 'FLOW:0x54607bd2c9da71d0' as UnionAddress,
                value: 100,
              },
            ]
          : [],
